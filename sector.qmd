...
.header-box {
  ...
  margin-bottom: 25px; border-bottom: 1px solid #334155; padding-bottom: 20px;
}
...
.dashboard-split {
  flex: 1; /* Ocupa el resto de altura */
  display: grid; grid-template-columns: 70% 28%; justify-content: space-between; gap: 20px; 
...
.comp-layout { display: grid; grid-template-columns: 25% 73%; gap: 2%; height: 100%; }
.comp-sidebar { display: flex; flex-direction: column; gap: 20px; }
.comp-main { background: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 20px; display: flex; flex-direction: column; height: 100%; }
...
// 1. DATOS ESTÁTICOS (AQUÍ ESTÁN LOS DATOS QUE SIEMPRE FUNCIONAN)
rawData = [
...
mutable activeCompetitors = ["A95758389", "B83160994"]; // Iberdrola, Axpo por defecto
...
<div class="main-wrapper">
  ...
  <div style="display: ${currentTab === 'mercado' ? 'flex' : 'none'}; animation: fadeIn 0.3s; flex-direction:column; height:100%;">
  ...
  <div style="display: ${currentTab === 'comparador' ? 'block' : 'none'}; animation: fadeIn 0.3s; height:100%;">
