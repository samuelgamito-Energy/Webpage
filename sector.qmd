...
<head>
    ...
    <title>Cuadro de Mando Estratégico FNEE</title>
    ...
    <style>
        /* Ajuste para que funcione dentro de Quarto */
        body { background-color: #f1f5f9 !important; font-family: 'Segoe UI', system-ui, sans-serif; margin: 0; }
        
        .tab-active { background-color: #1e293b; color: white; border-color: #1e293b; }
...
<body class="pb-10">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center gap-3">
                    <div class="bg-emerald-500 p-2 rounded text-slate-900"><i class="fa-solid fa-bolt"></i></div>
                    <div>
                ...
                <div class="flex space-x-2">
                    ...
                    <button onclick="switchTab('comparator')" id="btn-comparator" class="tab-inactive px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 hover:bg-slate-50">
                        <i class="fa-solid fa-chart-line"></i> Comparador
                    </button>
                    <button onclick="switchTab('data')" id="btn-data" class="tab-inactive px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 hover:bg-slate-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                <div class="card lg:col-span-2 overflow-hidden flex flex-col h-[600px]">
                    <div class="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                        <h3 class="font-bold text-slate-700"><i class="fa-solid fa-list mr-2"></i> Directorio de Empresas</h3>
                        <div class="relative">
                    ...
                    <div class="overflow-auto flex-1">
                        <table class="w-full text-sm text-left">
                            <thead class="text-xs text-slate-500 uppercase bg-slate-100 sticky top-0 z-10">
                                <tr>
                                    <th class="px-6 py-3 cursor-pointer" onclick="sortTable('cif')">CIF <i class="fa-solid fa-sort text-slate-300"></i></th>
                                    <th class="px-6 py-3 cursor-pointer" onclick="sortTable('name')">Razón Social <i class="fa-solid fa-sort text-slate-300"></i></th>
                                    <th class="px-6 py-3 text-right cursor-pointer" onclick="sortTable('sales2024')">Ventas '24 <i class="fa-solid fa-sort text-slate-300"></i></th>
                    ...
                    <div class="p-2 bg-slate-50 border-t text-center text-xs text-slate-400">
                        Mostrando las primeras 200 coincidencias por rendimiento
                    </div>
                </div>

                <div class="card p-4 flex flex-col">
                    <h3 class="font-bold text-slate-700 mb-4 text-sm">Volumen Total Mercado (GWh)</h3>
                    <div class="flex-1 relative">
                        <canvas id="marketChart"></canvas>
                    </div>
                    <div class="mt-4 p-3 bg-blue-50 rounded text-xs text-blue-800">
                        <i class="fa-solid fa-info-circle mr-1"></i> Evolución agregada de las ventas declaradas por todos los sujetos obligados cargados.
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="view-comparator" class="hidden fade-in">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[80vh]">
                <div class="lg:col-span-1 flex flex-col gap-4">
                    <div class="card p-5">
                        <h3 class="font-bold text-slate-800 mb-4 text-sm uppercase">1. Añadir Competidores</h3>
                        ...
                        <div id="selected-comps" class="space-y-2 mt-4 min-h-[100px]">
                            <p class="text-xs text-slate-400 italic text-center mt-4">Máximo 5 empresas</p>
                        </div>
                    </div>
                    
                    <div class="card p-5">
                        <h3 class="font-bold text-slate-800 mb-4 text-sm uppercase">2. Visualización</h3>
                        <button onclick="setChartMode('absolute')" class="w-full text-left px-4 py-3 rounded mb-2 text-sm font-medium transition-colors bg-blue-50 text-blue-700 border border-blue-200" id="mode-abs">
                            <i class="fa-solid fa-chart-bar mr-2"></i> Volumen Real (GWh)
                        </button>
                        <button onclick="setChartMode('indexed')" class="w-full text-left px-4 py-3 rounded text-sm font-medium transition-colors bg-white text-slate-600 border border-slate-200 hover:bg-slate-50" id="mode-idx">
                ...
                <div class="lg:col-span-3 card p-6 flex flex-col">
                    <h3 class="font-bold text-slate-700 mb-2">Comparativa Estratégica</h3>
                    <div class="flex-1 relative">
        ...
        <div id="view-data" class="hidden fade-in">
            <div class="max-w-2xl mx-auto mt-10">
                <div class="card p-10 border-dashed border-2 border-slate-300 hover:border-blue-500 transition-colors text-center cursor-pointer bg-slate-50"
                     id="drop-zone">
                    ...
                    <h3 class="text-xl font-bold text-slate-800">Motor de Ingesta de Datos</h3>
                    <p class="text-slate-500 mt-2 text-sm">Arrastra aquí tus archivos CSV (2019-2024).</p>
                    <p class="text-xs text-slate-400 mt-1">El sistema detectará el año por el nombre del archivo.</p>
                    <input type="file" id="file-input" multiple class="hidden" accept=".csv,.txt">
                    <button onclick="document.getElementById('file-input').click()" class="mt-6 bg-slate-900 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-slate-800">
    ...
    <script>
        ...
        // Se cargan por defecto para que la app funcione sin subir nada.
        let DB = [
        ...
        function switchTab(tabId) {
            ['dashboard', 'comparator', 'data'].forEach(t => {
                ...
                document.getElementById('btn-' + t).className = 'tab-inactive px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 hover:bg-slate-50 cursor-pointer';
            });
        ...
        function renderDashboard() {
            ...
            document.getElementById('kpi-total-cost').innerText = fmtEur(totalSales2024 * 239000).slice(0,-2) + ' M€'; // Estimacion basta
        ...
        function searchCompetitor() {
            ...
            if(matches.length > 0) {
                ...
                matches.forEach(c => {
                    ...
                    div.className = 'p-3 hover:bg-blue-50 cursor-pointer text-xs border-b border-slate-100';
        ...
        function renderSelectedChips() {
            ...
            container.innerHTML = '';
            const colors = ['#2563eb', '#db2777', '#16a34a', '#ea580c', '#7c3aed'];
        ...
        function setChartMode(mode) {
            comparatorMode = mode;
            document.getElementById('mode-abs').className = mode === 'absolute' ? 'w-full text-left px-4 py-3 rounded mb-2 text-sm font-medium transition-colors bg-blue-50 text-blue-700 border border-blue-200' : 'w-full text-left px-4 py-3 rounded mb-2 text-sm font-medium transition-colors bg-white text-slate-600 border border-slate-200 hover:bg-slate-50';
            document.getElementById('mode-idx').className = mode === 'indexed' ? 'w-full text-left px-4 py-3 rounded text-sm font-medium transition-colors bg-blue-50 text-blue-700 border border-blue-200' : 'w-full text-left px-4 py-3 rounded text-sm font-medium transition-colors bg-white text-slate-600 border border-slate-200 hover:bg-slate-50';
            updateComparatorChart();
        }
        ...
        function updateComparatorChart() {
            ...
            selectedCompetitors.forEach(cif => {
                ...
                const row = `
                    <tr class="border-b">
                        <td class="px-4 py-2 font-bold text-slate-700 truncate max-w-xs">${comp.name}</td>
                        <td class="px-4 py-2 text-right text-slate-500">${fmtNum(s20)}</td>
                        <td class="px-4 py-2 text-right font-bold">${fmtNum(s24)}</td>
                        <td class="px-4 py-2 text-right text-blue-600 font-bold">${cagr}%</td>
                        <td class="px-4 py-2 text-right ${growth >= 0 ? 'text-emerald-600' : 'text-rose-600'} font-bold">${growth}%</td>
                    </tr>
                `;
                tbody.innerHTML += row;
            });
        ...
        function openModal(cif) {
            ...
            document.getElementById('modal-cost').innerText = fmtEur(s24 * 239000); // 239k€/GWh aprox
        ...
        // --- 7. CARGA DE DATOS CSV (Drag & Drop) ---
        ...
        async function handleFiles(files) {
            ...
            let newDB = new Map();
            // Mantener datos existentes primero
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (year) {
                    ...
                    lines.forEach((line, idx) => {
                        ...
                        const parts = line.split(';'); // Asumiendo punto y coma
                        if(parts.length >= 3) {
                            let cif = parts[0].replace(/"/g, '').trim().toUpperCase();
                            let name = parts[1].replace(/"/g, '').trim();
                            let salesStr = parts[2].replace(/"/g, '').trim().replace('.','').replace(',','.'); // ES format to JS float
                            ...
                            if(name.length > entry.name.length) entry.name = name; // Mejor nombre
                        }
                    });

                    fileList.innerHTML += `<div class="text-sm text-emerald-600"><i class="fa fa-check"></i> ${file.name} procesado (${year})</div>`;
                }
            }

            DB = Array.from(newDB.values());
            renderDashboard(); // Refrescar UI
