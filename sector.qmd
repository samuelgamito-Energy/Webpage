...
/* Reset */
.dashboard-wrapper {
  font-family: 'Segoe UI', system-ui, sans-serif; color: #cbd5e1;
...
.tab-btn {
  ...
  color: #94a3b8; padding: 12px 25px; border-radius: 8px;
  cursor: pointer; font-weight: 600; display: flex; align-items: center; gap: 10px;
  transition: all 0.2s ease;
}
.tab-btn:hover { background: rgba(45, 212, 191, 0.1); color: #fff; }
.tab-btn.active {
  background: #0f172a; color: #2dd4bf; border: 1px solid #2dd4bf;
...
.glass-panel {
  ...
  box-shadow: 0 10px 30px -5px rgba(0, 0, 0, 0.3); backdrop-filter: blur(5px);
}
.panel-title { font-size: 1.3rem; font-weight: 700; color: #fff; margin-bottom: 5px; }
.panel-desc { font-size: 0.95rem; color: #94a3b8; margin-bottom: 25px; }

/* Tabla */
.table-container { overflow-x: auto; border-radius: 10px; border: 1px solid #334155; }
.custom-table { width: 100%; border-collapse: collapse; font-size: 0.95rem; }
.custom-table th {
  background: rgba(15, 23, 42, 0.9); color: #2dd4bf; padding: 15px; text-align: left;
  text-transform: uppercase; font-size: 0.8rem; letter-spacing: 1px;
  border-bottom: 2px solid #2dd4bf;
}
.custom-table td { padding: 15px; border-bottom: 1px solid #334155; color: #e2e8f0; }
.custom-table tr:hover { background: rgba(45, 212, 191, 0.05); }
...
.btn-view:hover { background: #2dd4bf; color: #0f172a; border-color: #2dd4bf; }

/* Modal */
dialog.chart-modal {
  background: #0f172a; color: #fff; border: 1px solid #2dd4bf; border-radius: 16px;
  padding: 0; width: 90%; max-width: 950px;
  box-shadow: 0 50px 100px -20px rgba(0,0,0,0.9);
  position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
}
dialog.chart-modal::backdrop { background: rgba(0,0,0,0.85); backdrop-filter: blur(4px); }
.modal-head {
  padding: 20px 30px; border-bottom: 1px solid #334155; background: rgba(30, 41, 59, 0.5);
  display: flex; justify-content: space-between; align-items: center;
}
.modal-body { padding: 30px; }
.btn-close { background: none; border: none; color: #94a3b8; font-size: 1.8rem; cursor: pointer; }
...
input[type="search"] {
  ...
  width: 100%; outline: none; transition: border 0.3s;
}
...
// DATOS INCRUSTADOS (Blindaje total: no fallará la carga)
csvData = `NIF;Empresa;Ventas 2021;Ventas 2022;Ventas 2023;Ventas 2024
...
// PROCESAMIENTO
data = d3.dsvFormat(";").parse(csvData, (d) => {
...
searchWidget = Inputs.search(data, { placeholder: "🔍 Buscar Endesa, Iberdrola...", columns: ["Empresa", "NIF"] });
...
<div class="dashboard-wrapper">
  ...
  <div class="tab-nav">
    <div class="tab-btn ${currentTab === 'mercado' ? 'active' : ''}" onclick=${() => mutable currentTab = 'mercado'}>
      <i class="fa-solid fa-chart-pie"></i> Mercado Global
    </div>
    <div class="tab-btn ${currentTab === 'comparador' ? 'active' : ''}" onclick=${() => mutable currentTab = 'comparador'}>
  ...
  <div style="display: ${currentTab === 'mercado' ? 'block' : 'none'}">
    <div class="glass-panel">
      <div class="panel-title">Evolución del Mercado</div>
      <div class="panel-desc">Volumen total agregado de las comercializadoras (GWh).</div>
  ...
  <div style="display: ${currentTab === 'comparador' ? 'block' : 'none'}">
    <div class="glass-panel">
      <div class="panel-title">Directorio de Empresas</div>
      <div class="panel-desc">Analiza el histórico individual de cada compañía.</div>
  ...
  if (selectedCompany) {
    const dialog = htl.html`<dialog class="chart-modal" open>
      <div class="modal-head">
        <div>
          <h2 style="margin:0; color:#2dd4bf; font-size:1.4rem;">${selectedCompany.Empresa}</h2>
          <span style="font-family:monospace; color:#94a3b8; font-size:1rem;">${selectedCompany.NIF}</span>
        </div>
        <button class="btn-close" onclick=${() => mutable selectedCompany = null}>&times;</button>
      </div>
      ...
      <div class="modal-body">
        ...
        <div style="margin-top:30px; display:flex; gap:15px; flex-wrap:wrap; justify-content:center;">
          ${years.map(y => htl.html`
            <div style="background:rgba(30,41,59,0.5); padding:15px 20px; border-radius:10px; border:1px solid #334155; text-align:center;">
              <div style="color:#64748b; font-size:0.85rem; margin-bottom:5px;">${y}</div>
